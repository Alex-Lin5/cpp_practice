...
public:
	...
	void operator=(const LinkedList<T>& L);//copy assignment; Lvalue assignment
	...
	void operator=(LinkedList<T>&& L);//Move assignment; Rvalue assighnment

};
template <typename T> void LinkedList<T>::operator=(LinkedList<T>&& L) {//Move assignment;
	while (head) {
		node<T>* p1{ head->next };
		delete head;
		head = p1;
...
template <typename T> LinkedList<T>  LinkedList<T>::ThreeTimes() {//ThreeTimes
	...
	return temp;//move wiill change a Lvalue object to Rvalue
...
template <class T> void LinkedList<T>::operator=(const LinkedList<T>& L) {//Copy Assignment
	if (this == &L) { cout << "Copy Assignment" << endl; return; }//L5 = L5;
	while (head) {
		node<T>* p1 = head->next;
		delete head;
		head = p1;
