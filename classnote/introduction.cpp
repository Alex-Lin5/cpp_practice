...
void f1(int i) { cout << "f1 by value" << endl;  ++i; }
/*
a new copy (although can be different name) is created for i and used inside the function.
Any change on i will not affect the original object outside the function.
*/



void f2(int& i) { cout << "f1 by reference" << endl;  ++i; }
/*
The same object outside the function is being used inside the function although under
a differnt name in the function.
Any modificaiton on i will actually modify w.
*/
...
int main() {
	...
	*/
	...
	delete p4;//deleting the object pointed by p4, rather than deleting p4.
	p4 = new int;
	*p4 = 56;
	delete p4;

	p4 = new int[5]{ 2,3,4,5,6 };
	delete[] p4;//always delete the whole array
	...
	int w{ 20 };
	f1(w);
	cout << w << endl;//20    call/pass by value
	f2(w);
	cout << w << endl;//21    call/pass by reference
